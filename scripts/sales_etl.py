...
spark = SparkSession.builder.appName("sales-order-view").getOrCreate()
...
df = df.withColumn("Quantity", F.expr("try_cast(Quantity as double)"))
...
result = df.groupBy("customer_name").agg(
    F.sum(F.col("quantity")).alias("per_customer_sales_quantity")
)
...
result.write.mode("overwrite").option("header", True).csv(output_file_path)
